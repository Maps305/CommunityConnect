...
propertyController.get('/find/types', async (req, res) => {
  try {
    const ruralType = await Property.countDocuments({ type: 'rural' })
    const urbanType = await Property.countDocuments({ type: 'urban' })
    const remoteType = await Property.countDocuments({ type: 'remote' })

    return res.status(200).json({ rural: ruralType, urban: urbanType, remote: remoteType })
  } catch (error) {
    return res.status(500).json(error)
  }
})
